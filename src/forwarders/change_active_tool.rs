...
use crate::{messages::{ChangeToolbarSlotMessage, SetActiveToolTypeMessage}, resources::Toolbar};
...
pub fn change_active_tool(
    ...
    for msg in change_toolbar_slot_mr.read() {
        let tooltype = toolbar.slots.get(msg.slot_index)
            .unwrap_or(&Some(ToolType::Select))
            .clone()
            .unwrap_or(ToolType::Select);
        set_active_tool_type_mw.write(SetActiveToolTypeMessage {
            tooltype
        });
    }
}
